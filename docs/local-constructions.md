...
These two constructions extend the *mathematical investigation*, and the [gyrobifastigium gallery](https://lukacslacko.github.io/axes/#gyrobifastigium) now includes all its initial cut-arrangement depths, with conservative colors backed by ordinary turns. The [elongated square gyrobicupola gallery](https://lukacslacko.github.io/axes/#gyrobicupola) also includes all 43 open arrangement regimes, 42 critical depths, and both endpoints. Its colors follow the same conservative policy; a singleton color does not prove that a piece can never transfer. Full state-dependent reachability for these local families is still separate work; the switches above do not settle those harder questions.
