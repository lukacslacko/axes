...
pending=sum(v.get('orbitStatus')=='regular-only' for f in standard for v in f['variants'])
for family in A['families']:
 for case in family['variants']:
  if family['key']!='jumble':
   ...
   if case.get('extraSwitches'):case['orbitMethod']='Verified ordinary permutations and legal partial-turn restoring loops; completeness/bounds recorded separately'
nav=[];sections=[]
...
for fi,f in enumerate(A['families']):
 ...
 for vi,v in sorted(enumerate(f['variants']),key=lambda x:x[1]['angle']):
  ...
  class_count=f'{len(v["geometricUpperBound"])}–{len(orbits)}' if uncertain else str(len(orbits))
  uncertainty='<p class="orbit-warning">Further mergers unresolved: same colors are verified reachable; different colors may still be equivalent. <a href="#orbit-bounds">About the bounds</a></p>' if uncertain else ''
...
<p class="reachability-status">@@PENDING@@ cases have unresolved color-class mergers; they show bounds and are marked individually. <a href="#orbit-bounds">What is established?</a></p>
...
<div id="orbit-bounds" class="local-constructions"><h3>Reachability: verified groups and unresolved mergers</h3><p>Most displayed cases have complete numerical reachability classifications. In @@PENDING@@ harder cases, the colors show groups linked by verified legal sequences, and a range gives the remaining bounds on the number of classes. Different colors in those marked cases are not a proof of inequivalence. The calculation examined at least 50,000 cut-diagram states per unresolved case.</p><p>The depth intervals classify the initial circle arrangement. Where partial turns occur, move availability may change at additional depths inside an interval; the colors and certificates apply to the angle actually drawn.</p></div>
<div id="local-constructions" class="local-constructions"><h3>Beyond the global rule</h3><p>Twisted polyhedra can support useful conical moves without a rotation symmetry about every vertex. Direct coordinate calculations give these two constructions:</p><div class="table-scroll"><table><thead><tr><th>Vertex rays</th><th>Example</th><th>Cut depth</th><th>Interacting moves</th></tr></thead><tbody><tr><td>8</td><td>Gyrobifastigium</td><td>40°</td><td>Four ordinary half-turn axes; four axes with 98.213° partial turns</td></tr><tr><td>24</td><td>Elongated square gyrobicupola</td><td>25°</td><td>Every axis supports a 64.737° partial turn</td></tr></tbody></table></div><p>A partial turn restores another axis while blocking some others. The eight-ray construction persists as its ridge angles vary continuously. These establish interacting moves; complete orbit-colored galleries for these extra families are not yet part of the finite catalogue.</p><p><a href="https://github.com/lukacslacko/axes/blob/main/docs/local-constructions.md">Coordinates, deformation formulas, and checked move witnesses ↗</a></p></div>
...
page=page.replace('@@PENDING@@',str(pending))
