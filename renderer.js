...
export function pieceAt(state,world,atlas){
 ...
 if(!id||masks[id]!==mask){for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){const xx=(x+dx+atlas.width)%atlas.width,yy=Math.max(0,Math.min(atlas.height-1,y+dy)),j=pixels[yy*atlas.width+xx];if(j&&masks[j]===mask)id=j;}if(!id||masks[id]!==mask)id=fallback[mask];}return id;
}
